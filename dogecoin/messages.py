...
from dogecoin.core.serialize import *
from dogecoin.net import *
import dogecoin
...
class MsgSerializable(Serializable):
    ...
    def to_bytes(self):
        ...
        self.msg_ser(f)
        body = f.getvalue()
        res = dogecoin.params.MESSAGE_START
        res += self.command
    ...
    def stream_deserialize(cls, f, protover=PROTO_VERSION):
        ...
        # check magic
        if recvbuf[:4] != dogecoin.params.MESSAGE_START:
            raise ValueError("Invalid message start '%s', expected '%s'" %
                             (b2x(recvbuf[:4]), b2x(dogecoin.params.MESSAGE_START)))
...
class msg_version(MsgSerializable):
    ...
    def __init__(self, protover=PROTO_VERSION):
        ...
        self.strSubVer = b'/python-dogecoin-0.0.1/'
