...
unlikely to match Satoshi Dogecoin exactly. Think carefully before using this
module.
"""
...
import dogecoin.core._bignum
import dogecoin.core.key
import dogecoin.core.serialize
...
class EvalScriptError(dogecoin.core.ValidationError):
...
def _CastToBigNum(s, err_raiser):
    v = dogecoin.core._bignum.vch2bn(s)
...
def _UnaryOp(opcode, stack, err_raiser):
    ...
    stack.append(dogecoin.core._bignum.bn2vch(bn))
    ...
    for (sop, sop_data, sop_pc) in scriptIn.raw_iter():
        ...
        if sop <= OP_PUSHDATA4:
            ...
            if sop == OP_1NEGATE or ((sop >= OP_1) and (sop <= OP_16)):
                v = sop - (OP_1 - 1)
                stack.append(dogecoin.core._bignum.bn2vch(v))
                ...
                check_args(1)
                stack.append(dogecoin.core.serialize.Hash160(stack.pop()))

            elif sop == OP_HASH256:
                check_args(1)
                stack.append(dogecoin.core.serialize.Hash(stack.pop()))
...
class VerifyScriptError(dogecoin.core.ValidationError):
    pass
...
class VerifySignatureError(dogecoin.core.ValidationError):
    pass
