...
# Copyright (C) 2011-2015 The python-dogecoinlib developers
#
...
import dogecoin
from dogecoin.core import COIN, lx, b2lx, CBlock, CTransaction, COutPoint, CTxOut
...
class RawProxy(object):
        ...
        if service_url is None:
            ...
            with open(doge_conf_file, 'r') as fd:
                ...
                if service_port is None:
                    service_port = dogecoin.params.RPC_PORT
                ...
                if 'rpcpassword' not in conf:
                    raise ValueError('The value of rpcpassword not specified in the configuration file: %s' % doge_conf_file)
    ...
    def __getattr__(self, name):
        ...
        # Make debuggers show <function dogecoin.rpc.name> rather than <function
        # dogecoin.rpc.<lambda>>
        f.__name__ = name
        return f
...
class Proxy(RawProxy):
    def __init__(self, service_url=None,
                       ...
                       timeout=DEFAULT_HTTP_TIMEOUT,
                       **kwargs):
        """Create a proxy to a dogecoin RPC service

        Unlike RawProxy data is passed as objects, rather than JSON. (not yet
        fully implemented) Assumes Dogecoin Core version >= 0.9; older versions
        mostly work, but there are a few incompatibilities.

        If service_url is not specified the username and password are read out
        of the file doge_conf_file. If doge_conf_file is not specified
        ~/.dogecoin/dogecoin.conf or equivalent is used by default. The default
        port is set according to the chain parameters in use: mainnet, testnet,
        or regtest.

        Usually no arguments to Proxy() are needed; the local dogecoind will be
        used.

        timeout - timeout in seconds before the HTTP interface times out
        """
        super(Proxy, self).__init__(service_url=service_url, service_port=service_port, doge_conf_file=doge_conf_file,
                                    timeout=timeout,
                                    **kwargs)
    ...
    def dumpprivkey(self, addr):
        ...
        return CDogecoinSecret(r)

    def getaccountaddress(self, account=None):
        """Return the current Dogecoin address for receiving payments to this account."""
        r = self._call('getaccountaddress', account)
        return CDogecoinAddress(r)
    ...
    def getnewaddress(self, account=None):
        """Return a new Dogecoin address for receiving payments.
    ...
    def getreceivedbyaddress(self, addr, minconf=1):
        ...
        addr    - The address. (CDogecoinAddress instance)
        minconf - Only include transactions confirmed at least this many times. (default=1)
        """
    ...
    def submitblock(self, block, params=None):
        ...
        params is optional and is currently ignored by dogecoind. See
        https://en.dogecoin.it/wiki/BIP_0022 for full specification.
        """
    ...
    def validateaddress(self, address):
        ...
        if r['isvalid']:
            r['address'] = CDogecoinAddress(r['address'])
        if 'pubkey' in r:
