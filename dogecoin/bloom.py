...
class CBloomFilter(dogecoin.core.serialize.Serializable):
    ...
    def insert(self, elem):
        ...
        """
        if isinstance(elem, dogecoin.core.COutPoint):
            elem = elem.serialize()
    ...
    def stream_deserialize(cls, f):
        vData = dogecoin.core.serialize.BytesSerializer.stream_deserialize(f)
        (nHashFuncs,
         nTweak,
    ...
    def stream_serialize(self, f):
        if sys.version > '3':
            dogecoin.core.serialize.BytesSerializer.stream_serialize(self.vData, f)
        else:
            # 2.7 has problems with f.write(bytearray())
            dogecoin.core.serialize.BytesSerializer.stream_serialize(bytes(self.vData), f)
